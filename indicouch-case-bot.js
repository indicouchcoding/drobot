...
function pickWear() { const w = weightedPick(CONFIG.wearTiers); const [min, max] = w.float; const fl = +(min + rng() * (max - min)).toFixed(4); return { ...w, float: fl }; }
function pickRarity() { const pool = CONFIG.rarities.slice().reverse(); return weightedPick(pool); }
...
function readPriceJSON(p, fallback) { try { return JSON.parse(fs.readFileSync(p, 'utf8')); } catch (_) { return fallback; } }
...
ensurePriceData();
// warm + refresh skinport feed
;(async () => { try { await PriceService._fetchSkinportItems(); } catch {} })();
setInterval(() => { PriceService._fetchSkinportItems().catch(() => {}); }, Math.max(PRICE_CFG.ttlMs || 600000, 300000));
...
function resolveCaseKey(input) {
  ...
  const aliases = { 'gamma':'Gamma Case','gamma case':'Gamma Case','gamma 2':'Gamma 2 Case','gamma2':'Gamma 2 Case' };
  ...
  let candidates = keys.filter(k=>{
    const nk = norm(k); const kHasDigit = /\d/.test(nk);
...
const client = new tmi.Client({
  ...
  channels: (process.env.TWITCH_CHANNEL || '').split(',').map(s => s.trim()).filter(Boolean),
});
...
client.connect().then(() => {
  ...
  console.log(`[indicouch:${INSTANCE_ID}] connected to`, (process.env.TWITCH_CHANNEL || '').split(','));
}).catch(console.error);
...
http.createServer((req, res) => {
  try {
    ...
    res.writeHead(200, { 'Content-Type': 'text/plain' }); res.end('Indicouch Case Bot OK');
  } catch (e) {
...
client.on('message', async (channel, tags, message, self) => {
  ...
  switch (cmd) {
    ...
    case 'open': {
      ...
      if (qtyIdx >= 0) { count = Math.max(1, Math.min(CONFIG.maxOpensPerCommand, parseInt(qtyMatch[1], 10))); args.splice(qtyIdx, 1); }
      ...
      // Sort hype: rarest → priciest → StatTrak → lower float
    ...
    case 'invlist': {
      ...
      if (args[1]) {
        ...
        if (tags.mod || badges.broadcaster === '1') targetUser = maybe; else { client.say(channel, `@${user} mods/broadcaster only to target another user.`); break; }
      }
      ...
      const basic = (d) => {
        ...
        const price = (typeof d.priceUSD==='number')?` • $${d.priceUSD.toFixed(2)}`:'';
        return `${parts.length?parts.join(' ')+' ':''}${d.weapon} | ${d.name} (${wearShort})${price}`;
      };
      const head = `@${user} [${rarityEmoji(rarityKey)} ${rarityKey}] ${items.length} item(s)` + (targetUser!==user?` for @${targetUser}`:'') + ':';
      await sayChunkedList(channel, head, items.map(basic));
      break;
    }
    ...
    case 'worth': {
      ...
      const { totalUSD, countValued, countAll } = await inventoryValue(target);

      if (countAll === 0) {
        if (isSelf) { client.say(channel, `@${user} you have an empty inventory.`); }
        else { client.say(channel, `@${user} @${target} has an empty inventory.`); }
        break; // IMPORTANT: break here too to avoid fallthrough
      }

      const msg = isSelf
        ? `@${user} inventory (excl. Blue): ${countValued}/${countAll} items • ~$${totalUSD.toFixed(2)} USD`
        : `@${user} @${target}'s inventory (excl. Blue): ${countValued}/${countAll} items • ~$${totalUSD.toFixed(2)} USD`;

      client.say(channel, msg);
      break; // IMPORTANT: prevents fallthrough to 'price'
    }
    ...
    case 'backupurl': {
      const badges = tags.badges || {};
      if (!(tags.mod || badges.broadcaster === '1')) { client.say(channel, `@${user} mods/broadcaster only.`); break; }
...
// kick off periodic refresh already set above; also clean disconnect
